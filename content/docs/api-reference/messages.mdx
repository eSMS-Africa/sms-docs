...
`cost` / `cost_currency` is the wallet deduction. `route_cost` / `route_currency` is the carrier's native price.

### Errors

| Code | Detail |
|------|--------|
| 400 | `"Invalid phone number: ..."` - phone cannot be parsed or is missing country code |
| 400 | `"No route found for ..."` - no active route for this country |
| 422 | `"Insufficient balance. Required: ... available: ..."` - wallet balance too low |
