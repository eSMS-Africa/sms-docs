...
queued → submitted → sent → delivered
                           ↘ failed (with auto-retry up to 2×)
```
...
The system automatically retries failed messages up to **2 times** before marking them permanently failed. The manual retry is in addition to automatic retries.

## Export to CSV

Download all messages (or filtered by status) as a CSV file:

```
GET /api/messages/export?status=failed
```

Returns a CSV with columns: `message_id`, `phone`, `sender_id`, `text`, `status`, `country`, `currency`, `cost`, `segments`, `created_at`, `delivered_at`.
